...
export default function BTreePage() {
  ...
  useEffect(() => {
    ...
    const {generatedKeys, keyRange } = generateKeys(INIT_BTREE_NKEYS,"random","number",[],false);
    ...
    switch (action) {
        ...
        switch (type) {
          ...
          default:
            ...
            setFormData((prevFormData) => ({
              ...
              importExportAreaValue : prevFormData.importExportDisplay == "export" && !btree.isEmpty() ? btree.export() : ""
            }));
            break;
        }
      ...
      case "generateGo":
        ...
        if (isNaN(n) || n < 1 || !Number.isInteger(n)) {
          ...
          if (btree.isEmpty()) {
            const { generatedKeys: keys, range: keyRange } = generateKeys(n,formData.generateKeyOrder,formData.generateKeyType,[],formData.allowDuplicates);
            generatedKeys = keys;
            range = keyRange;
          } else {
            ...
            const { generatedKeys: keys, range: keyRange } = generateKeys(n,formData.generateKeyOrder,type,existingKeys,formData.allowDuplicates);
            generatedKeys = keys;
            range = keyRange;
          }
          for(let i = 0; i < generatedKeys.length; i++){
      ...
      case "import":
        ...
        if (formData.importExportDisplay === "import" && formData.importExportAreaValue !== "") {
  ...
  return (
    <div className='btree-page-container'>
      ...
      <div className="btree-plot-container" ref={plotContainerRef}>
        {!btree.isEmpty() > 0 && (
          <BTreePlot treeData={treeData} treeProps={treeProps} plotProps = {plotProps}/>
        )}
      </div>
    </div>
  );
